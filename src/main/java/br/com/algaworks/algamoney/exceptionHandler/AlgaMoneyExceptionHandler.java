...
public class AlgaMoneyExceptionHandler extends ResponseEntityExceptionHandler{
	...
	public ResponseEntity<Object> DataIntegrityViolationException(DataIntegrityViolationException ex, WebRequest request){
		...
		return handleExceptionInternal(ex, new MensagemErro("Erro no SQL", mensagemDesenvolvedor), new HttpHeaders(), HttpStatus.CONFLICT, request);
	}
	...
	public ResponseEntity<Object> handleEmptyResultDataAcessException(EmptyResultDataAccessException ex, WebRequest request){
		...
		return handleExceptionInternal(ex, new MensagemErro(mensagemUsuario, mensagemDesenvolvedor), new HttpHeaders(), HttpStatus.NOT_FOUND, request);
	}
	
	@Override
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
			HttpHeaders headers, HttpStatus status, WebRequest request) {
		List<MensagemErro> erros = criaListaErros(ex.getBindingResult());
		return handleExceptionInternal(ex, erros, headers, HttpStatus.BAD_REQUEST, request);
	}
	
	private List<MensagemErro> criaListaErros(BindingResult bindingResult){
		
		List<MensagemErro> erros = new ArrayList<>();
		
		Arrays.asList(bindingResult.getFieldError())
			.stream()
			.forEach(erro -> erros.add(new MensagemErro(
